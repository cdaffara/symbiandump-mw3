...
EXPORT_C TInt CNSmlDMIAPMatcher::IAPIdFromURIL( const TDesC8& aURI )
	...
	TPtrC8 uriSlash = RemoveDotSlash(aURI);
	TPtrC8 uri = RemoveExtraChar(uriSlash);
...
// ------------------------------------------------------------------------------------------------
// TPtrC8 CNSmlDMIAPMatcher::RemoveExtraChar(const TDesC8& aURI) const
// ------------------------------------------------------------------------------------------------
TPtrC8 CNSmlDMIAPMatcher::RemoveExtraChar(const TDesC8& aURI) const
	{
	const TUint8 KNSmlDMUriSeparator = '/';
	const TUint8 KNSmlDMUriDot = '.';	
	
	TInt len = aURI.Length();
	if(len > 0)
	{
	if((aURI[0]==KNSmlDMUriSeparator) || (aURI[0]==KNSmlDMUriDot))
	{
		if((aURI[aURI.Length()-1]==KNSmlDMUriSeparator) && (len > 2))
		{
			return aURI.Mid(1,len-2);
		}
		else
			return aURI.Right(len-1);
	}
	else if(aURI[aURI.Length()-1]==KNSmlDMUriSeparator)
		{
		return aURI.Left( len -1 );
		}
	else
		return aURI;			
	}
