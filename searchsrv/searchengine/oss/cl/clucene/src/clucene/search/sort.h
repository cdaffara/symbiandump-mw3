...
//    ScoreDocComparator(CL_NS(util)::Comparable** cachedValues);
    ...
    struct RELEVANCE2_CREATOR { static ScoreDocComparator * create(); };
    DECL_STATMPTR1(ScoreDocComparator,RELEVANCE2_CREATOR,RELEVANCE2)
#define RELEVANCE2 GetInstancePtrScoreDocComparatorRELEVANCE2()
    
    struct INDEXORDER2_CREATOR { static ScoreDocComparator * create(); };
    DECL_STATMPTR1(ScoreDocComparator,INDEXORDER2_CREATOR,INDEXORDER2)
#define INDEXORDER2 GetInstancePtrScoreDocComparatorINDEXORDER2()
    
#else   
	/** Special comparator for sorting hits according to computed relevance (document score). */
	static ScoreDocComparator* RELEVANCE2;

	/** Special comparator for sorting hits according to index order (document number). */
	static ScoreDocComparator* INDEXORDER2;
#endif
 };
...
#if defined (__SYMBIAN32__) && defined (RVCT22_STATICS_WORKAROUND)
   struct SORTFIELD_CREATOR{ static SortField * create(int32_t type);};
   DECL_STATMPTR1(SortField,SORTFIELD_CREATOR,FIELD_SCORE,DOCSCORE)
#define FIELD_SCORE GetInstancePtrSortFieldFIELD_SCORE()
   
   DECL_STATMPTR1(SortField,SORTFIELD_CREATOR,FIELD_DOC,DOC)
#define FIELD_DOC GetInstancePtrSortFieldFIELD_DOC()
...
#if defined (__SYMBIAN32__) && defined (RVCT22_STATICS_WORKAROUND)
	struct RELEVANCE_CREATOR { static Sort * create(); };
	DECL_STATMPTR1(Sort,RELEVANCE_CREATOR,RELEVANCE)
#define RELEVANCE GetInstancePtrSortRELEVANCE()
	
	struct INDEXORDER_CREATOR { static Sort * create(); };
	DECL_STATMPTR1(Sort,INDEXORDER_CREATOR,INDEXORDER)
#define INDEXORDER GetInstancePtrSortINDEXORDER()
#else
