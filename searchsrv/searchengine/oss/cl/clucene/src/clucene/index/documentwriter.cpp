...
void DocumentWriter::writePostings(Posting** postings, const int32_t postingsLength, const char* segment){
	#define __DOCLOSE(obj) if(obj!=NULL){ try{ obj->close(); _CLDELETE(obj);} catch(LuceneError &e){ierr=e.number();err=e.what();} catch(std::exception &e) {err=e.what();} catch(...){err="Unknown error while closing posting tables";} }
